...
class Activity(commands.Cog):
    ...
    def __init__(self,bot):
        ...
        self.day_count = 1
        self.pending_weekly_report = None  # Store weekly report for approval
    ...
    async def get_daily_presence(self):
        ...
        if right_now.weekday() == 0:
            ...
            # Store the report and send to owner for approval
            self.pending_weekly_report = message
            owner = await self.bot.fetch_user(OWNER_USER_ID)
            await owner.send("📊 **Weekly Report Ready**\nHere's this week's voice chat activity:")
            await owner.send(message)
            await owner.send("Use `!publish_weekly` to post this to the public channel, or ignore to discard.")
            logger.info(f"Weekly report sent to owner for approval (Sunday)")
        else:
            owner = await self.bot.fetch_user(OWNER_USER_ID)
        hours_spent_dict = {}
    ...
    @commands.command(aliases=["previewweekly"], hidden=True)
    @commands.is_owner()
    async def weekly_preview(self, ctx):
        """
        Preview the weekly voice chat leaderboard since last Sunday.
        
        Shows total hours spent in voice chat by each user from Sunday to today,
        formatted exactly as it would appear in the weekly report.
        ...
        try:
            ...
            async with self.file_lock:
                ...
                if os.path.exists("data/dailyPresences.json"):
                    try:
                        ...
                        await ctx.send(f"❌ Failed to read daily presences: {e}")
                        return
                else:
                    await ctx.send("❌ No daily presences data found.")
                    return
            ...
            if not weekly_seconds:
                await ctx.send("📊 No activity data found for this week.")
                return
            ...
            # Format the message
            message = f"# 📊 Weekly Preview (Since {last_sunday.strftime('%B %d')})\n"
            message += f"*Showing data from {last_sunday.strftime('%Y-%m-%d')} to {today.strftime('%Y-%m-%d')}*\n\n"
            
            for person, seconds in sorted_weekly.items():
                hours = seconds / 3600
                message += f"**{person}** - `{hours:.2f} hours`\n"
            
            await ctx.send(message)
            logger.info(f"Weekly preview generated by {ctx.author.name} for {len(sorted_weekly)} users")
            
        except Exception as e:
            logger.error(f"Error in weekly_preview command: {e}", exc_info=True)
            await ctx.send(f"❌ Error generating weekly preview: {e}")

    @commands.command(aliases=["postweekly", "sendweekly"], hidden=True)
    @commands.is_owner()
    async def publish_weekly(self, ctx):
        """
        Publish the pending weekly report to the public channel.
        
        Posts the weekly voice chat activity report that was sent to you
        for approval. The report is automatically generated on Sundays.
        ...
        ctx : commands.Context
            The command context.
        """
        if not self.pending_weekly_report:
            await ctx.send("❌ No pending weekly report to publish. The report is generated automatically on Sundays.")
            return
        
        try:
            channel = await self.bot.fetch_channel(WEEKLY_CHANNEL_ID)
            await channel.send(self.pending_weekly_report)
            await ctx.send(f"✅ Weekly report published to <#{WEEKLY_CHANNEL_ID}>!")
            logger.info(f"Weekly report published by {ctx.author.name}")
            self.pending_weekly_report = None  # Clear the pending report
        except Exception as e:
            logger.error(f"Error publishing weekly report: {e}", exc_info=True)
            await ctx.send(f"❌ Error publishing weekly report: {e}")
